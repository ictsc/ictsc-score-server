...
module Sinatra
  module NestedEntityHelpers
    ...
    # rubocop:disable Metrics/MethodLength, Lint/NonLocalExitFromIterator
    ...
    def filter_entities(member:, resource:, entities:, parent_entity: nil)
      ...
      entities.each.with_index do |entity, i|
        ...
        when Array
          ...
          return
        when Hash
          unless model.readables(user: member, action: action).exists?(id: r[entity]['id'])
            r.delete(entity)
            return
          end
          filter_columns(member: member, entities: [r[entity]], model: model)
    ...
    # rubocop:enable Metrics/MethodLength, Lint/NonLocalExitFromIterator
    ...
    # @param action []
    # @return
    def as_json_of(klass, nested_params:, by:, as_option: {}, id: nil, action: '', where: {}) # rubocop:disable Metrics/ParameterLists
    ...
    def generate_nested_hash(klass:, by:, params:, id: nil, as_option: {}, apply_filter: true, action: '', where: {}) # rubocop:disable Metrics/ParameterLists
